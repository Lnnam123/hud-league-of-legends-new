...
<template>
  <div
    class="w-full h-full flex items-center py-1 px-1 gap-4"
    :class="mirror ? 'flex-row-reverse' : 'flex-row'"
  >
    <MatchScore
      class="self-stretch"
      :best-of="bestOf"
      :fill-color="mirror ? 'var(--red-team-color)' : 'var(--blue-team-color)'"
    ...
    >
      ...
      <p class="font-bold overflow-clip whitespace-nowrap text-ellipsis text-sm text-white!">
        {{ team.infoText }}
      </p>
    </div>
    <TextWithIcon
